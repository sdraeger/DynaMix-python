...
class Dataset:
    ...
    def __init__(
        ...
        if (
            ...
            or not isinstance(test, torch.Tensor)
        ):
            raise TypeError(
                "data, context, and test must be numpy arrays or torch.Tensor objects"
            )

        # Validate input shapes
        if len(data.shape) != 3:
            raise ValueError(
                f"Expected data to have 3 dimensions (seq_length, num_sequences, N), got shape {data.shape}"
            )

        if len(context.shape) != 3:
            raise ValueError(
                f"Expected context to have 3 dimensions (context_length, num_sequences, N), got shape {context.shape}"
            )

        if len(test.shape) != 3:
            raise ValueError(
                f"Expected test to have 3 dimensions (seq_length, num_systems, N), got shape {test.shape}"
            )

        # Validate compatibility between tensors
        if data.shape[1] != context.shape[1]:
            raise ValueError(
                f"Number of sequences in data ({data.shape[1]}) and context ({context.shape[1]}) must match"
            )

        if data.shape[2] != context.shape[2] or data.shape[2] != test.shape[2]:
            raise ValueError(
                f"Feature dimension N must be consistent: data ({data.shape[2]}), context ({context.shape[2]}), test ({test.shape[2]})"
            )

        # Store the data on the specified device with the specified dtype
        self.X = (
            data.clone().detach().to(device=self.device, dtype=self.dtype)
        )  # Training data: (seq_length, num_sequences, N)
        self.context = (
            context.clone().detach().to(device=self.device, dtype=self.dtype)
        )  # Context data: (context_length, num_sequences, N)
        self.test = (
            test.clone().detach().to(device=self.device, dtype=self.dtype)
        )  # Test data: (seq_length, num_systems, N)
    ...
    def __getitem__(self, idx):
        ...
        if idx < 0 or idx >= self.num_sequences:
            raise IndexError(
                f"Index {idx} out of bounds for dataset with {self.num_sequences} sequences"
            )
    ...
    def sample_batch(self):
        ...
        Context = torch.stack(Context).permute(
            1, 0, 2
        )  # Shape: (context_length, batch_size, N)
