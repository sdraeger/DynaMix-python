...
def plot_3D_attractor(
    ...
    for i in range(3):
        ...
        ax_ts = fig.add_axes(
            [left_positions[1] + 0.02, y_pos, widths[1] - 0.05, height]
        )
    ...
    for i in range(3):
        ...
        ax_ps = fig.add_axes(
            [left_positions[2] + 0.02, y_pos, widths[2] - 0.05, height]
        )
        ...
        if ground_truth is not None:
            gt_data = (
                ...
                if isinstance(ground_truth[:, i], np.ndarray)
                else ground_truth[:, i].detach().cpu().numpy().flatten()
            )
        ...
        pred_data = (
            ...
            if isinstance(prediction[:, i], np.ndarray)
            else prediction[:, i].detach().cpu().numpy().flatten()
        )
        ps_gen = compute_and_smooth_power_spectrum(pred_data, smoothing_sigma)

        # Plot prediction power spectrum
        ax_ps.plot(
            ps_gen[:lim_pse], label="DynaMix", linewidth=2.5, color="#FF4242", alpha=0.9
        )
