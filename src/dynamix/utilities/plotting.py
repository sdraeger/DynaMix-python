...
def plot_trajectories(
    real_traj, pred_traj, save_dir=None, name_prefix="", plot_length=1000
):
    """
...
def plot_metrics(metrics_data, save_dir=None, name_prefix="", show=False):
    ...
    if losses is not None:
        ...
        x_axis = (
            ...
            if epochs is None or len(epochs) != len(losses)
            else epochs
        )
    ...
    if metrics_to_plot and epochs:
        ...
        for i, (name, metric) in enumerate(metrics_to_plot):
            ...
            # Convert tensors to numpy if needed
            metric_np = [m.cpu().numpy() if hasattr(m, "cpu") else m for m in metric]
            plt.plot(epochs, metric_np, color="#2C3E50", linewidth=2.5, alpha=0.9)
