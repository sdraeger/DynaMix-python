...
def train_dynamix(
    model, dataset, optimizer, scheduler, args, printing=True, plotting=True
):
    """
    ...
    # Testing hyperparameters - adapt to test data size
    test_length = dataset.test.shape[0]
    context_length = min(500, test_length // 2)  # Use half for context, max 500
    prediction_steps = test_length - context_length  # Predict the rest

    # Use shorter prediction horizon during training for faster evaluation
    # Full horizon used only for final evaluation
    eval_prediction_steps = min(100, prediction_steps)
    plot_id = 0

    # Setup checkpoint directories using the utility function
    save_dir, checkpoint_dir, plots_dir = create_checkpoint_directories(
        args.save_path, args
    )
    ...
    for e in epochs:
        ...
        for batch_idx in range(batches_per_epoch):
            ...
            z_hat = predict_sequence_using_gtf(
                model, x, context, alpha, n_interleave
            )  # Predict sequence using teacher forcing

            # Calculate loss
            loss = loss_function(model, z_hat, y, lambda_reg=lambda_reg)
        ...
        # Calculate training duration (before evaluation)
        training_duration = time.time() - epoch_start_time

        # Compute and store average loss for the epoch
        average_epoch_loss = sum(epoch_losses) / len(epoch_losses)
        losses.append(average_epoch_loss)

        # Print training time for first few epochs
        if e < 3:
            print(f"  Epoch {e} training time: {training_duration:.2f}s", flush=True)
        ...
        if e % ssi == 0:
            with torch.no_grad():
                ...
                # Select subset of trajectories for efficient evaluation
                n_eval = min(10, dataset.test.shape[1])
                eval_indices = torch.randperm(dataset.test.shape[1])[:n_eval]

                # Generate predictions only for subset with reduced horizon for speed
                forecaster = DynaMixForecaster(model)
                X_gen = forecaster.forecast(
                    dataset.test[0:context_length, eval_indices, :],
                    eval_prediction_steps,
                )

                # Move tensors to CPU for metrics calculation
                X_gen_cpu = X_gen.cpu() if X_gen.is_cuda else X_gen
                test_data_cpu = (
                    dataset.test[:, eval_indices, :].clone().detach().cpu()
                    if dataset.test.is_cuda
                    else dataset.test[:, eval_indices, :].clone().detach()
                )
                ...
                for eval_id in range(n_eval):
                    klx_values.append(
                        geometrical_misalignment(
                            X_gen_cpu[:, eval_id, :],
                            test_data_cpu[
                                context_length : context_length + eval_prediction_steps,
                                eval_id,
                                :,
                            ],
                            n_bins=n_bins,
                ...
                # Store metrics
                klx.append(
                    torch.nanmedian(torch.tensor(klx_values, device=model.B.device))
                )
                dh.append(
                    torch.nanmedian(torch.tensor(dh_values, device=model.B.device))
                )
                mase_values.append(
                    torch.nanmedian(
                        torch.tensor(mase_batch_values, device=model.B.device)
                    )
                )
                ssi_epochs.append(e)
                ...
                if plotting:
                    ...
                    plot_trajectories(
                        real_trajectory, predicted_trajectory, plots_dir, f"epoch_{e}_"
                    )

                # Switch back to train mode
                model.train()

            # Calculate full epoch duration (including evaluation if it happened)
            epoch_duration = time.time() - epoch_start_time

            # Store epoch time for SSI epochs (for ETA calculation)
            if e % ssi == 0:
            ...
            if printing:
                ...
                if e % ssi == 0:
                    # Full output with metrics for SSI epochs
                    # Calculate estimated remaining time based on SSI epochs
                    ...
                    print(
                        f"Epoch {e} loss: {average_epoch_loss:.6f} | Epoch time: {time_str} | ETA: {eta_str} | Dstsp: {klx[-1]:.6f} | DH: {dh[-1]:.6f} | MASE: {mase_values[-1]:.6f}",
                        flush=True,
                    )
                elif e % 10 == 0:
                    # Brief progress update every 10 epochs
                    print(
                        f"Epoch {e} loss: {average_epoch_loss:.6f} | {time_str}",
                        flush=True,
                    )
    ...
    with torch.no_grad():
        ...
        forecaster = DynaMixForecaster(model)
        X_gen = forecaster.forecast(
            dataset.test[0:context_length, :, :], prediction_steps
        )

        # Move tensors to CPU for plotting
        X_gen_cpu = X_gen.cpu() if X_gen.is_cuda else X_gen
        real_trajectory = dataset.test[
            context_length : context_length + prediction_steps, plot_id, :
        ].cpu()
