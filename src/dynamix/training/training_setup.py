...
def setup_gpu(args):
    ...
    if args.device == "cuda":
        ...
        if args.gpu_id >= num_gpus:
            print(
                f"WARNING: GPU {args.gpu_id} requested but only {num_gpus} GPUs available. Using GPU 0."
            )
            args.gpu_id = 0
        ...
        print(
            f"GPU Memory: {torch.cuda.get_device_properties(args.gpu_id).total_memory / 1e9:.2f} GB"
        )
    else:
...
def load_settings(settings_file):
    ...
    try:
        ...
        print(
            f"Warning: Settings file {settings_file} not found. Using default settings."
        )
        return {}
    except json.JSONDecodeError:
        print(
            f"Warning: Settings file {settings_file} is not valid JSON. Using default settings."
        )
        return {}
...
def parse_args():
    ...
    parser.add_argument(
        "--latent_dim",
        type=int,
        default=model_settings.get("latent_dim", 10),
        help="Dimension of latent state",
    )
    parser.add_argument(
        "--hidden_dim",
        type=int,
        default=model_settings.get("hidden_dim", 50),
        help="Dimension of hidden state for clipped shallow PLRNN",
    )
    parser.add_argument(
        "--experts",
        type=int,
        default=model_settings.get("experts", 10),
        help="Number of experts",
    )
    parser.add_argument(
        "--pwl_units",
        type=int,
        default=model_settings.get("pwl_units", 2),
        help="Number of PWL units",
    )
    parser.add_argument(
        "--expert_type",
        type=str,
        default=model_settings.get("expert_type", "almost_linear_rnn"),
        choices=["almost_linear_rnn", "clipped_shallow_plrnn"],
        help="Type of expert architecture to use",
    )
    parser.add_argument(
        "--probabilistic_expert",
        action="store_true",
    ...
    parser.add_argument(
        "--batch_size",
        type=int,
        default=training_settings.get("batch_size", 16),
        help="Batch size for training",
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=training_settings.get("epochs", 2000),
        help="Number of epochs for training",
    )
    parser.add_argument(
        "--alpha",
        type=float,
        default=training_settings.get("alpha", 1.0),
        help="Alpha parameter for teacher forcing",
    )
    parser.add_argument(
        "--n_interleave",
        type=int,
        default=training_settings.get("n_interleave", 10),
        help="Interleave parameter for teacher forcing",
    )
    parser.add_argument(
        "--start_lr",
        type=float,
        default=training_settings.get("start_learning_rate", 2e-3),
        help="Starting learning rate",
    )
    parser.add_argument(
        "--end_lr",
        type=float,
        default=training_settings.get("end_learning_rate", 1e-5),
        help="Ending learning rate",
    )
    parser.add_argument(
        "--batches_per_epoch",
        type=int,
        default=training_settings.get("batches_per_epoch", 50),
        help="Number of batches per epoch",
    )
    parser.add_argument(
        "--ssi",
        type=int,
        default=training_settings.get("scalar_saving_interval", 20),
        help="Scalar saving interval",
    )
    parser.add_argument(
        "--noise_level",
        type=float,
        default=training_settings.get("noise_level", 0.05),
        help="Noise level for training data",
    )
    parser.add_argument(
        "--reg_strength",
        type=float,
        default=training_settings.get("regularization_strength", 0.01),
    ...
    parser.add_argument(
        "--n_bins",
        type=int,
        default=ssd_settings.get("n_bins", 30),
        help="Number of bins for state space divergence",
    )
    parser.add_argument(
        "--ps_smoothing",
        type=int,
        default=ps_settings.get("smoothing", 20),
        help="Smoothing parameter for power spectrum",
    )
    parser.add_argument(
        "--mase_steps",
        type=int,
        default=mase_settings.get("steps", 20),
    ...
    parser.add_argument(
        "--threads",
        type=int,
        default=system_settings.get("threads", 4),
        help="Number of threads to use for computation",
    )
    parser.add_argument(
        "--seed", type=int, default=system_settings.get("seed", 42), help="Random seed"
    )
    parser.add_argument(
        "--device",
        type=str,
        default=system_settings.get("device", "cpu"),
        choices=["cpu", "cuda"],
    ...
    parser.add_argument(
        "--gpu_id",
        type=int,
        default=system_settings.get("gpu_id", 0),
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default=paths_settings.get("data_path", ""),
        help="Path to training data",
    )  # expected to be in form of a ($T-T_C+Δt+1$, $S$, $N$) NumPy array (.npy)
    parser.add_argument(
        "--context_path",
        type=str,
        default=paths_settings.get("context_path", ""),
        help="Path to context data",
    )  # expected to be in form of a ($T_C$, $S$, $N$) NumPy array (.npy)
    parser.add_argument(
        "--test_path",
        type=str,
        default=paths_settings.get("test_path", ""),
        help="Path to test data",
    )  # Optional: expected to be in form of a ($T$, $S$, $N$) NumPy array (.npy)
    parser.add_argument(
        "--save_path",
        type=str,
        default=paths_settings.get("save_path", "results"),
...
def training_setup():
    ...
    # Load all data
    print(
        f"Loading data from {args.data_path} (ground truth), {args.context_path} (context), {args.test_path} (test)"
    )
    ...
    print(
        f"Data shape: {dataset.X.shape}, Context shape: {dataset.context.shape}, Test shape: {dataset.test.shape}"
    )
    ...
    if args.expert_type == "almost_linear_rnn":
        model = DynaMix(
            ...
            probabilistic_expert=args.probabilistic_expert,
        ).to(device)
    ...
    # Note: torch.compile causes significant overhead for RNN-style sequential loops
    # due to compilation time on each new input shape. Disabled for now.
