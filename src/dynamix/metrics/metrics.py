...
def geometrical_misalignment_single(x_gen, x_true, n_bins=30):
    ...
    for i in range(x_true.shape[1]):
        min_, max_ = (
            x_true[:, i : i + 1].min(0).values,
            x_true[:, i : i + 1].max(0).values,
        )
        ...
        hist_true = calc_histogram(
            x_true[:, i : i + 1], n_bins=n_bins, min_=min_, max_=max_
        )

        p_gen = normalize_to_pdf_with_laplace_smoothing(
            histogram=hist_gen, n_bins=n_bins
        )
        p_true = normalize_to_pdf_with_laplace_smoothing(
